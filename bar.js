...
var zAlt = d3.scaleOrdinal()
    .range(d3.schemeCategory20c.slice(8, 12).reverse())
...
function drawBars(trainTest, dataPoint) {
  d3.csv("data/" + trainTest + "/data_" + dataPoint + ".csv", function(d, i, columns) {
      ...
      g.append("g")
          ...
          .enter().append("rect")
          .attr("class", "bar")
          .attr("id", function(d) { return d.value[1]; })
          .attr("x", function(d) { return x1(d.key); })
          .attr("y", function(d) {
              return y(d.value[0]); })
          .attr("width", x1.bandwidth())
          .attr("height", function(d) { return barHeight - y(d.value[0]); })
          .attr("fill", function(d) { return pickColor(d.key, d.value[1]); })
